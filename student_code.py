...
class LeNet(nn.Module):
    def __init__(self, input_shape=(32, 32), num_classes=100):
        ...
        self.linear_1 = nn.Linear(400, 256)
        self.linear_2 = nn.Linear(256, 128)
        self.linear_3 = nn.Linear(128, 100)

    def forward(self, x):
        leng = len(x)
        shape_dict = {1: [leng, 6, 14, 14], 2: [
            leng, 16, 5, 5], 3: [leng, 400], 4: [leng, 256],
            5: [leng, 128], 6: [leng, 100]}
        # certain operations
...
def train_model(model, train_loader, optimizer, criterion, epoch):
    ...
    print('[Training set] Epoch: {:d}, Average loss: {:.4f}'.format(
        epoch+1, train_loss))
